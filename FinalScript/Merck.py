...
f.write("*******************************************************************************")
f.write("\nResults printed below are for Regression Data set 10 Merck \n" )
f.write("*******************************************************************************\n")
f.write("\n")
...
#     grid_search = GridSearchCV(SVR(), param_grid, cv=nfolds, n_jobs=-1)
    grid_search = SVR()
    grid_search.fit(X, y)
    f.write('\nSVR MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
...
def random_forest_regressor_param_selection(X, y, X_test, y_test, nfolds):
    ...
    grid_search.fit(X, y)
    f.write('\nRandomForestRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
    ...
    y_pred = grid_search.predict(X_test)
    f.write('\nMAE for RandomForestRegressor on test set: '+str(mean_absolute_error(y_test, y_pred)))
...
def decision_tree_regressor_param_selection(X, y, X_test, y_test, nfolds):
    ...
    grid_search.fit(X, y)
    f.write('\nDecisionTreeRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
    ...
    y_pred = grid_search.predict(X_test)
    f.write('\nMAE for DecisionTreeRegressor on test set: '+str(mean_absolute_error(y_test, y_pred)))
...
def ada_boost_regressor_param_selection(X, y, X_test, y_test, nfolds):
    ...
    grid_search.fit(X, y)
    f.write('\nAdaBoostRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
    ...
    y_pred = grid_search.predict(X_test)
    f.write('\nMAE for AdaBoostRegressor on test set: '+str(mean_absolute_error(y_test, y_pred)))
...
#     grid_search.fit(X, y)
#     f.write('\nGaussianProcessRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
...
#     y_pred = grid_search.predict(X_test)
#     f.write('\nMAE for GaussianProcessRegressor on test set: '+str(mean_absolute_error(y_test, y_pred)))
...
def linear_regressor_param_selection(X, y, X_test, y_test, nfolds):
    ...
    grid_search.fit(X, y)
    f.write('\nLinearRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
...
def neural_network_regressor_param_selection(X, y, X_test, y_test, nfolds):
    ...
    grid_search.fit(X, y)
    f.write('\nNeuralNetworkRegressor MAE Score for training data: '+str(grid_search.score(X_test, y_test)))
    ...
    y_pred = grid_search.predict(X_test)
    f.write('\nMAE for NeuralNetworkRegressor on test set: '+str(mean_absolute_error(y_test, y_pred)))
...
f.write("----------------------------------------------------------------------------------------------------------------------")
f.write("\nAccording to our methods, Decision tree regressor gave us the lowest MAE on test data but as the train error is quite low it is unlikely that this is the best model. We suggest either Neural network Regressor or the Adaboost regressor which have comparable train and test accuracy(Neural Network = 51% and Adaboost = 58%)")
f.write("\n----------------------------------------------------------------------------------------------------------------------")
